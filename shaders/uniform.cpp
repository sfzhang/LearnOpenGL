...
int main()
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        float green_color = (sin(glfwGetTime()) / 2.0f) + 0.5f;
        int vertex_color_loc = glGetUniformLocation(shader_program, "vertex_color");
        if (vertex_color_loc != -1) {
            glUseProgram(shader_program);
            glUniform4f(vertex_color_loc, 0.0f, green_color, 0.3f, 1.0f);
        }
